...
<input type="text" name="title" class="rounded bg-gray-200 w-full mb-4" value="{{old('title', $post->title)}}">

<label for="" class="uppercase text-gray-700 text-xs">URL</label>
<span class="text-xs text-red-600">@error('slug') {{$message}}  @enderror</span>
<input type="text" name="slug" class="rounded bg-gray-200 w-full mb-4" value="{{old('slug', $post->slug)}}">

<label for="" class="uppercase text-gray-700 text-xs">Contenido</label>
<span class="text-xs text-red-600">@error('body') {{$message}}  @enderror</span>
<textarea name="body" rows="5" class="rounded bg-gray-200 w-full mb-4">{{old('body', $post->body)}}</textarea>
